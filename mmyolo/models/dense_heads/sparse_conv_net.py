...
def sparse_gn(x, gn, pw_x):
    ...
    x_part = x_part * gn.weight.unsqueeze(dim=0).unsqueeze(dim=2).unsqueeze(dim=3) + gn.bias.unsqueeze(dim=0).unsqueeze(
...
class Sparse_conv2d(torch.autograd.Function):
    ...
    def forward(ctx, input, hard, weights, bias, stride, padding, isbias, base, gn=None, pw=None, nonzero_hard=None):
        ...
        if str(input.device) == 'cpu':
            assert not str(
                input.device) == 'cpu', 'we do not support CPU inference, you can try codes in sparse_conv_cpu folder, but we cannot ensure the correctness'
            ...
            output = \
            sparse_conv.forward(input, hard, weights, bias, stride[0], padding[0], isbias, base, groups, gnweight,
                                gnbias, pw_mean, pw_rstd, eps, nonzero_hard[0], nonzero_hard[1])[0]
...
class SparseConv2d(torch.nn.modules.conv._ConvNd):
    ...
    def _fast_forward(self, input, hard, pw, weight, bias, isbias, base, gn):
        if hard.n_keep == 0:
            one_ = torch.ones((input.shape[0], self.out_channels, input.shape[2], input.shape[3]), dtype=input.dtype,
                              device=input.device)
