...
class PPYOLOECSPPAFPNADD(BaseYOLONeck):
    """CSPPAN in PPYOLOE.
    ...
    def __init__(self,
        ...
        self.up_sample = self.out_channel_upsample_layer(int(out_channels[0] * widen_factor))
        self.down_sample = self.out_channel_down_sample_layer(int(out_channels[-1] * widen_factor))
        # print('上采样:{}'.format(self.up_sample))
        # print('下采样:{}'.format(self.down_sample))
    ...
    def out_channel_upsample_layer(self, in_channels: int) -> nn.Module:
        """build upsample layer."""
        # fpn_route
        return nn.Sequential(
            ConvModule(
    ...
    def out_channel_down_sample_layer(self, in_channels: int) -> nn.Module:
        """build upsample layer."""
        # fpn_route
    ...
    def forward(self, inputs: List[torch.Tensor]) -> tuple:
        ...
        for idx in range(len(self.in_channels)):
            ...
            # print('显示这个结果：{}'.format(inputs[idx].shape))
        ...
        outs0 = self.up_sample(outs[0])
        outs_last = self.down_sample(outs[-1])
        ...
        for i in range(self.num_heads):
            xx = []
            for ii in range(3):
                xx.append(results[i+ii])
            results11.append(xx)
        # print(results11)
        return tuple(results11)
        # for xxx in results:
        #     print('显示这个结果：{}'.format(xxx.shape))
