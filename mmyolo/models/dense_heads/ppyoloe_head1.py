...
class DCNv3_block(torch.nn.Module):
    def __init__(self, channels, group):
        super(DCNv3_block, self).__init__()
        self.dcnv3 = DCNv3(channels=channels, group=group)
...
class PPYOLOEHeadModuleMY(BaseModule):
    ...
    def _init_layers(self):
        ...
        for i, in_channel in enumerate(self.in_channels):
            ...
            self.block.append(SimAM_module(in_channel))
    ...
    def forward(self, x: Tuple[Tensor]) -> Tensor:
        ...
        return multi_apply(self.forward_single, x, self.cls_stems,
                           ...
                           self.block_down, self.block, self.block_up, self.dcnv)
    ...
    def forward_single(self, x: Tensor, cls_stem: nn.ModuleList,
                       ...
                       block_down: nn.ModuleList, block: nn.ModuleList, block_up: nn.ModuleList, dcnv: nn.ModuleList) -> Tensor:
...
@MODELS.register_module()
class PPYOLOEHeadMY(YOLOv6Head):
